...
export async function GET(req: Request) {
  try {
    ...
    if (!to) {
      return new Response(JSON.stringify({ ok: false, error: "missing ?to" }), { status: 400 });
    }
    const apiKey = process.env.SENDGRID_API_KEY;
    const from = process.env.DELIVERY_FROM_EMAIL || "no-reply@example.com";
    if (!apiKey) {
      return new Response(JSON.stringify({ ok: false, error: "SENDGRID_API_KEY missing" }), { status: 500 });
    }
    const body = {
